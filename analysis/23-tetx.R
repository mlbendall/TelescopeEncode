...
################################################################################
# TETranscripts - DESeq2 (PolyA extract only)
################################################################################
...
mincount <- 5
minsamp  <- 2
gfilt <- rowSums(counts.tetx[,sfilt] >= mincount) >= minsamp

dds.sel <- DESeqDataSetFromMatrix(counts.tetx[gfilt, sfilt], samples[sfilt, ], ~celltype)
...
################################################################################
# TETranscripts - Hierarchical clustering samples
################################################################################
...
if(makepdf) pdf('analysis/figures/tetx.sample_clustering.pdf', 
...
samples.pv %>% 
    ...
    hang.dendrogram() %>%
    plot(main="TETranscripts - Cluster dendrogram with AU/BP values")
...
################################################################################
# TETranscripts  - Differentially expressed genes
################################################################################
...
if(makepdf) pdf('analysis/figures/tetx.celltype_deg.pdf', 
...
p12 <- pca.df %>% 
    ...
    ggtitle('TETranscripts - HERV expression PCA')

if(makepdf) pdf('analysis/figures/tetx.sample_pca_1v2.pdf', paper='USr', width=10, height=8)
p12
if(makepdf) dev.off()
...
if(makepdf) pdf('analysis/figures/tetx.sample_pca_1v3.pdf', paper='USr', width=10, height=8)
p13
if(makepdf) dev.off()
...
save(tetx, file='analysis/Rdata/tetx.Rdata')
