...
################################################################################
# Unique counts - DESeq2 (PolyA extract only)
################################################################################
...
mincount <- 5
minsamp  <- 2
gfilt <- rowSums(counts.unique[,sfilt] >= mincount) >= minsamp
gfilt <- gfilt & (annot.herv$chrom != 'chrY')

dds.sel <- DESeqDataSetFromMatrix(counts.unique[gfilt, sfilt], samples[sfilt, ], ~celltype)
sizeFactors(dds.sel) <- calculateSizeFactors(metrics[sfilt,]$unique.ts) # Use UNIQUE counts
...
################################################################################
# Unique counts - Hierarchical clustering samples
################################################################################
...
if(makepdf) pdf('analysis/figures/ucounts.sample_clustering.pdf', 
...
samples.pv %>% 
    ...
    hang.dendrogram() %>%
    plot(main="Unique Counts - Cluster dendrogram with AU/BP values")
...
################################################################################
# Unique counts - Differentially expressed genes
################################################################################
...
if(makepdf) pdf('analysis/figures/ucounts.celltype_deg.pdf', 
...
save(unique_counts, file='analysis/Rdata/unique_counts.Rdata')
