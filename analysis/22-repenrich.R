...
################################################################################
# RepEnrich - DESeq2 (PolyA extract only)
################################################################################
...
mincount <- 5
minsamp  <- 2
gfilt <- rowSums(counts.repenrich[,sfilt] >= mincount) >= minsamp

dds.sel <- DESeqDataSetFromMatrix(counts.repenrich[gfilt, sfilt], samples[sfilt, ], ~celltype)
...
################################################################################
# RepEnrich - Hierarchical clustering samples
################################################################################
...
if(makepdf) pdf('analysis/figures/repenrich.sample_clustering.pdf', 
...
samples.pv %>% 
    ...
    hang.dendrogram() %>%
    plot(main="RepEnrich - Cluster dendrogram with AU/BP values")
...
################################################################################
# RepEnrich - Differentially expressed genes
################################################################################
...
if(makepdf) pdf('analysis/figures/repenrich.celltype_deg.pdf', 
...
p12 <- pca.df %>% 
    ...
    ggtitle('RepEnrich - HERV expression PCA')

if(makepdf) pdf('analysis/figures/repenrich.sample_pca_1v2.pdf', paper='USr', width=10, height=8)
p12
if(makepdf) dev.off()
...
if(makepdf) pdf('analysis/figures/repenrich.sample_pca_1v3.pdf', paper='USr', width=10, height=8)
p13
if(makepdf) dev.off()
