...
################################################################################
# Telescope family level - DESeq2 (PolyA extract only)
################################################################################
...
mincount <- 5
minsamp  <- 2
gfilt <- rowSums(counts.fam[,sfilt] >= mincount) >= minsamp

dds.sel <- DESeqDataSetFromMatrix(counts.fam[gfilt, sfilt], samples[sfilt, ], ~celltype)
...
################################################################################
# Telescope family level - Hierarchical clustering samples
################################################################################
...
if(makepdf) pdf('analysis/figures/tfam.sample_clustering.pdf', 
...
samples.pv %>% 
    ...
    hang.dendrogram() %>%
    plot(main="Telescope (family level) - Cluster dendrogram with AU/BP values")
...
################################################################################
# Telescope family level  - Differentially expressed genes
################################################################################
...
if(makepdf) pdf('analysis/figures/tfam.celltype_deg.pdf', 
...
p12 <- pca.df %>% 
    ...
    ggtitle('Telescope (family level) - HERV expression PCA')

if(makepdf) pdf('analysis/figures/tfam.sample_pca_1v2.pdf', paper='USr', width=10, height=8)
p12
if(makepdf) dev.off()
...
if(makepdf) pdf('analysis/figures/tfam.sample_pca_1v3.pdf', paper='USr', width=10, height=8)
p13
if(makepdf) dev.off()
...
save(tfam, file='analysis/Rdata/tfam.Rdata')
